...
const notes =
  (x: number, y: number, dx: number, dy: number) => (view: View) => {
    let evt: DeepEvent = []
    for (let i = 0; i < 20; i++) {
      evt.push([view.particle('Note', x, y, dx, dy), Events.wait(8)])
    }
    return Events.flatten(evt)
  }

const rings =
  (x: number, y: number, dx: number, dy: number, skip: number = 6) =>
  (view: View) => {
    let evt: DeepEvent = []
    for (let i = 0; i < 10; i++) {
      evt.push([view.particle('Ring', x, y, dx, dy), Events.wait(skip)])
    }
...
const iceBeam =
  ...
  (view: View) => {
    let evt = []
    const p = (stage: PIXI.Container) =>
      new Particle.Static(stage, x, y, 'ICE_3', 19)
        .transformX((t: number) => dx)
        .transformY((t: number) => dy)
    for (let i = 0; i < 15; i++) {
      evt.push([
        view.particleV1(p),
        Events.wait(2),
...
const reflect = (x: number, y: number) => (view: View) => {
  ...
  const pieces: [
    ...
    [
      ['BEAM_2_END', 2, 0, true, true],
      ['BEAM_2', 4, 0.125],
    ],
    [
      ['BEAM_2_END', 1, 0, true, true],
      ['BEAM_2', 3, 0.125],
      ['BEAM_2', 4, 1.125],
    ],
    [
      ['BEAM_2_END', 0, 0, true, true],
      ['BEAM_2', 2, 0.125],
      ['BEAM_2', 3, 1.125],
      ['BEAM_2', 4, 2.125],
    ],
    [
      ['BEAM_2_CORNER', 0, 0, true, true],
      ['BEAM_2', 1, 0],
      ['BEAM_2', 2, 1],
      ['BEAM_2', 3, 2],
      ['BEAM_2', 4, 3],
  ...
  for (let i = 0; i < pieces.length; i++) {
    ...
    ps.forEach(([tex, ox, oy, flipX, flipY]) => {
      const createParticle = (stage: PIXI.Container) => {
        let particle = new Particle.Static(
          stage,
          x + ox * 8,
          y + oy * 8,
          i % 2 === 0 ? tex : YELLOW_TO_ORANGE_BEAM[tex],
          lifespan
        )
          .delayStart(i * lifespan)
          .setXAnchor(0)
          .setYAnchor(0)
        if (flipX) {
...
const scratch = (x: number, y: number) => (view: View) =>
  Events.flatten([
...
const rage = (x: number, y: number, isPlayer: boolean) => (view: View) =>
  Events.flatten([
...
const sleepAnim = (isPlayer: boolean, sfx: boolean) => (view: View) => {
  const [x, y, dx] = isPlayer ? [54, 68, -1] : [108, 32, 1]
  return Events.flatten([
...
const crunch = (x: number, y: number) => (view: View) =>
  Events.flatten([
    view.invertColors(),
    view.particleV1(stage =>
      new Particle.Static(stage, x + 8, y - 8, 'BOOM_BIG', 8).delayStart(8)
    ),
    view.particleV1(stage =>
      new Particle.Static(stage, x - 8, y + 8, 'BOOM_BIG', 8).delayStart(22)
    ),
    view.particleV1(stage =>
      new Particle.Static(stage, x, y - 48, 'CRUNCH_1', 30).transformY(
        (t: number) => {
          const SPEED = 5.0
    ...
    view.particleV1(stage =>
      new Particle.Static(stage, x, y + 32, 'CRUNCH_2', 30).transformY(
        (t: number) => {
          const SPEED = -3.0
          if (t < 0.25) return SPEED
          if (t < 0.25 + 0.125) return 0
...
const jumpKick = (x: number, y: number) => (view: View) => {
  return Events.flatten([
    view.particleV1(stage =>
      new Particle.Static(stage, x - 6 + 2 * 16, y - 6 - 16, 'FOOT', 24)
        .transformX(t => -2)
        .transformY(t => 1)
    ),
    view.particleV1(stage =>
      new Particle.Static(stage, x + 6 + 2 * 16, y + 6 - 16, 'FOOT', 20)
        .transformX(t => -2)
        .transformY(t => 1)
    ),
    Events.wait(20),
...
const growl =
  ...
  (view: View) => {
    ...
    for (let i = 0; i < times; i++) {
      evt.push([view.particle('Growl', x, y, dir), Events.wait(16)])
    }
    return Events.flatten(evt)
  }
...
const encore = (x: number, y: number) => (view: View) => {
  return Events.flatten([
    view.particleV1(stage =>
      new Particle.Static(stage, x, y, 'CLAP', 66).offsetX(
        t => 16 * -Math.abs(Math.cos(2 * Math.PI * t))
      )
    ),
    view.particleV1(stage =>
...
const earthquake = (view: View) =>
  Events.flatten([view.screenShake(120, 5, 4, false), Events.wait(90)])
...
const burned =
  (x: number, y: number, dir: number = 1) =>
  (view: View) => {
    const seq: Particle.AttackTexture[] = [
      'FIRE_SMALL_1',
      'FIRE_SMALL_2',
      'FIRE_BIG_1',
      'FIRE_BIG_2',
    ...
    for (let i = 0; i < 4; i++) {
      ...
      e.push(
        view.particleV1(stage =>
          new Particle.Sequence(stage, x, y, seq, i * 2, 20)
...
const longTwinkle = (x: number, y: number) => (view: View) =>
  Events.flatten([
...
const fireBlast =
  (x1: number, y1: number, x2: number, y2: number) => (view: View) => {
    ...
    const lastFlame = (dir: [number, number], delay: number = 0) =>
      view.particleV1(stage =>
        new Particle.Sequence(
          stage,
          x2,
...
function sinkOpp(view: View, times: number = 1, duration: number = 30): TEvent {
  return {
    init: state => {
      ...
      view.getOpponentSprite().texture = new PIXI.Texture(
        tex.baseTexture,
        new PIXI.Rectangle(
          0,
          0,
...
function thunderbolt(x: number, y: number): (view: View) => TEvent {
  ...
  return view =>
    Events.flatten([
      ...
      view.particleV1(stage =>
        new Particle.Static(stage, x - 16, y - 16, 'SMALL_THUNDER_3', 125)
          .setFlicker(THUNDER_FLICKER_1)
          .delayStart(25)
      ),
      view.particleV1(stage =>
        new Particle.Static(stage, x - 24, y - 24, 'SMALL_THUNDER_3', 125)
          .setFlicker(THUNDER_FLICKER_2)
          .delayStart(25)
      ),
      ...
      view.particleV1(stage =>
        new Particle.Static(
          ...
          y - 20,
          'SMALL_THUNDER_1',
          125 - THUNDER_FLICKER_1
        )
          .setFlicker(THUNDER_FLICKER_1)
          .delayStart(25 + THUNDER_FLICKER_1)
      ),
      view.particleV1(stage =>
        new Particle.Static(
          stage,
          ...
          y - 28,
          'SMALL_THUNDER_1',
          125 - THUNDER_FLICKER_2
        )
          .setFlicker(THUNDER_FLICKER_2)
          .delayStart(25 + THUNDER_FLICKER_2)
      ),
...
function thunder(x: number, y: number): (view: View) => TEvent {
  ...
  const area = (w: number) => (t: number) =>
    new PIXI.Rectangle(
      ...
      0,
      w,
      t < STRIKE / LIFE ? Math.floor((((t * LIFE) / STRIKE) * 56) / 8) * 8 : 56
    )
  return view =>
    Events.flatten([
      view.particleV1(stage =>
        new Particle.Static(stage, x + 14, y, 'THUNDER_SIDE', LIFE)
          .subArea(area(24))
          .unanchorY()
          .delayStart(15)
      ),
      ...
      view.particleV1(stage =>
        new Particle.Static(stage, x, y, 'THUNDER_MID', LIFE)
          .subArea(area(16))
          .unanchorY()
          .delayStart(30)
      ),
...
const swift =
  (
    ...
    delay: number,
    dir: number = 1,
    texture: Particle.AttackTexture = 'STAR',
    spread: number = 0,
    xDist: number = 40,
    spins: number = 1,
    rotSpeed: number = 1
  ...
  (view: View) => {
    ...
    return Events.flatten([
      view.particleV1(stage =>
        new Particle.Static(stage, x, y, texture, initialLife)
          .delayStart(delay)
          .offsetX(t => t * xDist * speed * dir)
          .offsetY(yPos)
          .priority(1)
      ),
      view.particleV1(stage =>
        new Particle.Static(
          ...
          .delayStart(delay + initialLife)
          .offsetX(
            t =>
              -loopXRad +
              Math.cos(2 * Math.PI * t * spins * rotSpeed) * loopXRad
          )
          .offsetY(
            t => dir * -Math.sin(2 * Math.PI * t * spins * rotSpeed) * loopYRad
          )
          .priority(1)
      ),
      view.particleV1(stage =>
        new Particle.Static(
          ...
          .delayStart(delay + initialLife + loopLife)
          .offsetX(t => t * xDist * speed * dir)
          .offsetY(yPos)
          .priority(1)
      ),
...
const mudslap =
  ...
  (view: View) => {
    ...
    const dist = 120 - 54
    const particle =
      (index: number, delay: number) => (stage: PIXI.Container) => {
        const p = new Particle.Static(stage, x, y, 'DIRT_SHADOW', 12)
          .delayStart(delay)
          .offsetX(t => dir * dist * t)
          .offsetY(t => -dir * t * (24 + index * 12))
...
const weather =
  (
    texture: Particle.AttackTexture = 'SHINE_LIGHT',
    spread: number = 0,
    count: number = 100
  ) =>
  (view: View) => {
...
const solarBeamCharge = (x: number, y: number) => (view: View) => {
  ...
  const charge1 = (dx: number, dy: number) => {
    return view.particleV1(stage =>
      new Particle.Sequence(
        stage,
        x + dx,
        y + dy,
        ['SOLAR_BEAM_1', 'SOLAR_BEAM_2', 'SOLAR_BEAM_3', 'SOLAR_BEAM_3'],
        28,
        80
      )
        .offsetX(t => t * -dx)
  ...
  return Events.flatten([
    ...
    view.particleV1(
      stage =>
        new Particle.Sequence(
          ...
          x,
          y,
          ['SOLAR_BEAM_4', 'SOLAR_BEAM_5', 'SOLAR_BEAM_6'],
          33,
          99
        )
    ),
    ...
    Events.wait(25),
    view.particleV1(stage =>
      new Particle.Static(stage, x, y, 'SOLAR_BEAM_6', 33).setFlicker(4)
    ),
    Events.wait(40),
  ])
}
      ...
      evt.push(
        view.particleV1(stage =>
          new Particle.Static(
            ...
            x + dir * (4 + 8 * (i + 1)),
            y + dir * (-2 - 4 * (i + 1)),
            i % 2 === 0 ? 'LASER_END_1' : 'LASER_END_2',
            delay
          )
            .delayStart(delay * i)
            .flipHorizontally(dir < 0)
            .flipVertically(dir < 0)
        )
      )
      evt.push(
        view.particleV1(stage =>
...
const firespin =
  (x: number, y: number, dir: number = 1, xDist: number = 40) =>
  (view: View) => {
    const evt: DeepEvent = []
    for (let i = 0; i < 8; i++) {
      evt.push(
        swift(
          ...
          i * 4,
          dir,
          i % 2 === 0 ? 'FIRE_SMALL_1' : 'FIRE_SMALL_2',
          Math.random() * 32 - 16,
          xDist
        )(view)
      )
...
const blizzard =
  (x: number, y: number, dir: number = 1, xDist: number = 40) =>
  (view: View) => {
    const ices: Particle.AttackTexture[] = ['ICE_1', 'ICE_2', 'ICE_3', 'ICE_4']
    const evt: DeepEvent = []
    for (let i = 0; i < 9; i++) {
      evt.push(
        swift(
...
const sunnyDay = (view: View) =>
  Events.flatten([view.brighten(), weather()(view), view.resetMatrixFilter()])

const rainDance = (view: View) =>
  Events.flatten([
    view.darken(),
...
const surf = (view: View) => {
  ...
  return Events.flatten([
    ...
    view.particleV1(stage =>
      new Particle.Static(stage, -8 * 9, 96, 'SURF', totalTime)
        .unanchorX()
        .offsetX(xPos)
        .offsetY(yPos)
    ),
    view.particleV1(stage =>
      new Particle.Static(stage, -8 * 9 + 80, 96 - 8, 'SURF', totalTime)
        .unanchorX()
        .offsetX(xPos)
        .offsetY(yPos)
    ),
    view.particleV1(stage =>
      new Particle.Static(stage, -8 * 9 + 160, 96 - 16, 'SURF', totalTime)
        .unanchorX()
        .offsetX(xPos)
        .offsetY(yPos)
    ),
    ...
    tornado.push(
      view.particleV1(stage =>
        new Particle.Sequence(
          ...
          x,
          y - i * 8,
          ['TORNADO_1', 'TORNADO_2', 'TORNADO_3'],
          4,
          lifetime - 36 - i * betweenTornados
        )
          .delayStart(36 + betweenTornados * i)
          .offsetX(t => Math.sin(2 * Math.PI * t * 15) * ((8 * i) / 7))
      )
    )
  }
...
const effects: { [attack: string]: Effect } = {
  ...
  SNORE_PRE: {
    ply: view =>
      Events.flatten([view.particle('Z', 54, 68, -1), Events.wait(20)]),
    opp: view =>
      Events.flatten([view.particle('Z', 108, 32, 1), Events.wait(20)]),
  },
  ...
  CHARM: {
    ply: view =>
      Events.flatten([
        view.particleV1(stage =>
          new Particle.Static(stage, 56, 64, 'HEART', 36)
            .offsetY(t => -t * 24)
            .offsetX(t => Math.sin(2 * Math.PI * t) * 8)
        ),
        {
          done: t => {
            view.getPlayerSprite().x =
    ...
    opp: view =>
      Events.flatten([
        view.particleV1(stage =>
          new Particle.Static(stage, 96, 32, 'HEART', 36)
            .offsetY(t => -t * 24)
            .offsetX(t => Math.sin(2 * Math.PI * t) * 8)
        ),
        {
  ...
  'BODY SLAM': {
    ply: view =>
      Events.flatten([
        view.particleV1(stage =>
          new Particle.Static(stage, 128, 28, 'BOOM_MED', 6).delayStart(
            TACKLE_DUR
          )
        ),
        view.particleV1(stage =>
          new Particle.Static(stage, 128 + 8, 28, 'BOOM_MED', 6).delayStart(
            TACKLE_DUR + 6
          )
        ),
        {
    ...
    opp: view =>
      Events.flatten([
        view.particleV1(stage =>
          new Particle.Static(stage, 34, 72, 'BOOM_MED', 6).delayStart(
            TACKLE_DUR
          )
        ),
        view.particleV1(stage =>
          new Particle.Static(stage, 34 - 8, 72, 'BOOM_MED', 6).delayStart(
            TACKLE_DUR + 6
          )
        ),
        {
  ...
  'BODY SLAM_PRE': {
    ply: view => Events.flatten([sinkPly(view)]),
    opp: view => Events.flatten([sinkOpp(view)]),
  },
  ...
  CRUNCH: {
    opp: crunch(ATTACK_PLY_X, ATTACK_PLY_Y),
    ply: crunch(ATTACK_OPP_X - 4, ATTACK_OPP_Y),
  },
