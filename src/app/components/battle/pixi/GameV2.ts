...
function buildPSTeam(
  members: MemberObject[],
  fighters: { [id: string]: FighterObject }
): PokemonSet[] {
  return members.map((member, index) => {
    const fighter = fighters[member.id]
    if (fighter == null) {
      throw new Error(
        `GameV2.buildPSTeam: member id "${member.id}" does not exist`
      )
    }
    return {
...
class GameV2 implements IGame {
  ...
  constructor(
    ...
    const playerTeam = PSTeams.pack(
      buildPSTeam(this.battleInfo.info.player.team, this.battleInfo.data)
    )
    const opponentTeam = PSTeams.pack(
      buildPSTeam(this.battleInfo.info.opponent.team, this.battleInfo.data)
    )
    streams.omniscient.write(
      `>player p1 ${JSON.stringify({
        name: battleInfo.info.player.name,
        team: playerTeam,
      })}`
    )
    streams.omniscient.write(
      `>player p2 ${JSON.stringify({
        name: battleInfo.info.opponent.name,
        team: opponentTeam,
      })}`
    )
  ...
  switch(index: number) {
    ...
    this.eventDriver.append(
      Events.flatten([
        ...
        Events.wait(24),
        () =>
          this.streams.p1.write(
            `switch ${this.battle.p1.pokemon.indexOf(member) + 1}`
          ),
      ])
    )
  }
  ...
  getSimulatedPlayer(): Pokemon {
    ...
    if (poke == null) {
      throw new Error(
        'GameV2.getSimulatedPlayer: simulator player member is null'
      )
    }
    return poke
  }
  ...
  getSimulatedOpponent(): Pokemon {
    ...
    if (poke == null) {
      throw new Error(
        'GameV2.getSimulatedOpponent: simulator opponent member is null'
      )
    }
    return poke
  }
  ...
  getSimulatedPlayerMember(index: number): Pokemon {
    return this.battle.p1.pokemon.find(
      pkmn => pkmn.set === this.battle.p1.team[index]
    )!
  }

  getSimulatedOpponentMember(index: number): Pokemon {
    return this.battle.p2.pokemon.find(
      pkmn => pkmn.set === this.battle.p2.team[index]
    )!
  }
