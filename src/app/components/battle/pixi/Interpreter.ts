  ...
  private step(game: IGame | null, command?: Script): DeepEvent {
    ...
    switch (command.do) {
      case 'ANIMATE': {
        ...
        if (member == null) {
          throw new Error(
            `Interpreter.step: bad index, team.length=${this.battleInfo.info.opponent.team.length} command=${JSON.stringify(command)}`
          )
        }
        id = member.id
        ...
        if (data == null) {
          throw new Error(
            `Interpreter.step: bad id, id=${id} command=${JSON.stringify(command)}`
          )
        }
      ...
      case 'HEALTH':
        return Events.changeHealth(
          command.isPlayer
            ? this.view.getPlayerStatus()
            : this.view.getOpponentStatus(),
          command.hp,
      ...
      case 'SET_PLAYER': {
        ...
        if (member == null) {
          throw new Error(
            `Interpreter.step: bad index, team.length=${this.battleInfo.info.player.team.length} command=${JSON.stringify(command)}`
          )
        }
      ...
      default:
        throw new Error(
          `Unsupported script command: ${JSON.stringify(command)}.`
        )
    }
  }
