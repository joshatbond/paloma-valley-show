...
abstract class Menu {
  ...
  compose(s: number, g: () => void) {
    ...
    if (f == null) {
      throw new Error(
        `Menu.compose: bad state, state=${this.state} states=${this.states}`
      )
    }
    this.machine[s] = () => {
      f()
  ...
  update() {
    ...
    if (xy == null) {
      throw new Error(
        `Menu.update: bad state, state=${this.state} arrowPos=${JSON.stringify(this.arrowPos)}`
      )
    }
  ...
  listen() {
    ...
    if (f == null) {
      throw new Error(
        `Menu.listen: bad state, state=${this.state} states=${this.states}`
      )
    }
    f()
  }
  ...
  show() {
    if (!this.showing) {
      ...
      if (xy == null) {
        throw new Error(
          `Menu.show: bad state, state=${this.state} arrowPos=${JSON.stringify(this.arrowPos)}`
        )
      }
      this.arrowSpr.x = xy[0]
      this.arrowSpr.y = xy[1]
    ...
    this.compose(0, () => {
      if (Input.selected()) {
        ...
        if (simulatedPlayer.moveSlots.every(move => move.pp <= 0)) {
          ...
          const moveIndex = simulatedPlayer.moveSlots.findIndex(
            move => move.move === simulatedPlayer.lastMove?.name
          )
          this.game.popAllMenus()
    ...
    this.compose(3, () => {
      if (Input.selected()) {
        ...
        game
          ...
          .append(
            Events.flatten([
              game.view.text(["No! There's no", 'running from a', 'battle!']),
              game.view.clearTextbox(),
              Events.wait(10),
              () => this.show(),
            ])
...
class Moves extends Menu {
  ...
  constructor(game: GameV2, moves: string[], state: number) {
    ...
    if (moves.every(m => m === ''))
      throw new Error(
        'Moves.constructor: Cannot create move menu with no moves!'
      )
    ...
    if (trans.length > 1) trans.push([lastMove!, trans[0]![0], 'down'])
    super(
      game,
      trans,
      [
  ...
  listen() {
    ...
    if (Input.back()) {
      ...
      if (move == null) {
        const simulatedMoves = JSON.stringify(
          player.moveSlots.map(move => move.move)
        )
        throw new Error(
          `Moves.listen: bad move, moveIndex=${moveIndex} player.moveSlots=${simulatedMoves} moves=${JSON.stringify(this.moves)}`
        )
      }
      if (move.disabled) {
        this.game
          ...
          .append(
            Events.flatten([
              this.game.view.text(['This move is', 'DISABLED!']),
              this.game.view.clearTextbox(),
            ])
          )
      } else if (move.pp <= 0) {
        this.game
          ...
          .append(
            Events.flatten([
              this.game.view.text(["There's no PP left", 'for this move!']),
              this.game.view.clearTextbox(),
            ])
          )
      } else {
  ...
  updateMoveInfo() {
    ...
    if (moveIndex == null) {
      throw new Error(
        `Moves.updateMoveInfo: bad state, state=${this.state} states=${this.states} moves=${JSON.stringify(this.moves)}`
      )
    }
    ...
    if (stats == null) {
      const simulatedMoves = JSON.stringify(
        simulatedMoveSlots.map(move => move.move)
      )
      throw new Error(
        `Moves.updateMoveInfo: bad move, moveIndex=${moveIndex} player.moveSlots=${simulatedMoves} moves=${JSON.stringify(this.moves)}`
      )
    }
    ...
    this.type.change(move.type.toUpperCase())
  ...
  _show() {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (this.moves[i] == null) {
        throw new Error(
          `Moves._show: moves should have length 4, but was ${JSON.stringify(this.moves)}`
        )
      }
      if (move == null) {
        this.moves[i]!.clear()
...
function createTeamViewTransitions(
  length: number
...
function createTeamViewTouchAreas(length: number): PIXI.Rectangle[] {
  const areas: PIXI.Rectangle[] = [new PIXI.Rectangle(0, 0, 160, 23)]
...
abstract class TeamView extends Menu {
  ...
  update() {
    ...
    this.iconSpr.forEach((s, i) => {
      ...
      if (memberInfo == null) {
        throw new Error(
          `TeamView.update: bad icon, index=${i} team.length=${this.game.battleInfo.info.player.team.length}`
        )
      }
      const icon = this.game.battleInfo.data[memberInfo.id]?.icon || 0
...
class SwitchoutTeamView extends TeamView {
  ...
  handle() {
    ...
    if (member.hp <= 0) {
      ...
      if (this.roar) {
        this.selectMessage([
          `${GameV2.getTrueName(member.name)}`,
          'is too afraid!',
        ])
      } else {
        this.selectMessage([
