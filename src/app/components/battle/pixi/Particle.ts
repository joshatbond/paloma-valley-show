...
class Particle {
  ...
  update() {
    ...
    if (
      ...
      Math.floor((this.timer - this.delayFrames) / this.flickerFrames) % 2 === 0
    ) {
      this.flicker()
    }
    this.timer++
  ...
  flipHorizontally(condition: boolean = true) {
    if (condition) this.sprites[0].scale.x = -this.sprites[0].scale.x
    return this
  }

  flipVertically(condition: boolean = true) {
    if (condition) this.sprites[0].scale.y = -this.sprites[0].scale.y
    return this
  }
...
class Static extends Particle {
  ...
  _update() {
    if (this.subAreaFn != null) {
      const newArea = this.subAreaFn(
        (this.timer - this.delayFrames) / this.life
      )
...
class Sequence extends Particle {
  ...
  _update() {
    ...
    this.sprites[0].texture =
      this.texs[
        Math.floor((this.timer - this.delayFrames) / this.delay) %
          this.texs.length
      ]
  }
...
class Growl extends Particle {
  ...
  _update() {
    ...
    this.sprites[0].texture = this.sprites[2].texture = dark
...
class Note extends Particle {
  ...
  _update() {
    ...
    this.sprites[0].y = Math.floor(
      this.y + Math.sin((this.timer * this.dx) / 24) * 8
    )
    if (
      this.x < 0 ||
      this.y < 0 ||
...
class Z extends Particle {
  ...
  _update() {
    ...
    this.x += this.dx / 16.0
    this.y -= 0.25
    this.sprites[0].x = Math.floor(this.x + Math.sin(this.timer / 6) * 6)
    this.sprites[0].y = Math.floor(this.y)
...
class Paralysis extends Particle {
  ...
  _update() {
    // 120 frames
    this.sprites[0].x =
      this.x + (Math.floor(this.timer / 6) % 2 === 0 ? -2 : 2) * this.dir
    this.sprites[0].texture =
      Math.floor(this.timer / 2) % 2 === 0 ? attackTex.PARA_1 : attackTex.PARA_2
    if (this.timer >= 120) this.die()
  }
}
...
class ShadowBall extends Static {
  ...
  _update() {
    ...
    this.sprites[0].x = Math.floor(this.x)
    this.sprites[0].y = Math.floor(
      this.y - 16 * Math.sin((this.timer * 2.0 * Math.PI) / 20.0)
    )
  }
}
...
class RisingIceWall extends Particle {
  ...
  _update() {
    if (this.timer < this.riseTime) {
      ...
      if (
        Math.floor(((this.timer - this.riseTime) / this.flickerTime) * 4) %
          2 ===
        0
      ) {
        if (!this.wait) {
