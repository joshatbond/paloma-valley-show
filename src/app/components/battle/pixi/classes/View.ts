...
function animate(
  ...
  for (let i = 0; i < ref.length; i++) {
    ...
    if (d == null) {
      throw new Error(
        `View.animate: delay is null, frame=${i} delay.length=${delay.length}`
      )
    }
    script.push({
...
export class View implements IView {
  ...
  showOpponentStats(member?: MemberObject): Event {
    ...
    return {
      init: () => this.opponentStats.show(showMember, this.opponentStatus),
    }
  }
  ...
  setStatus(isPlayer: boolean, status: Partial<Status>) {
    if (isPlayer) {
      ...
      this.playerStatus.condition =
        status.condition ?? this.playerStatus.condition
    } else {
      this.opponentStatus.maxHp = status.maxHp ?? this.opponentStatus.maxHp
      this.opponentStatus.hp = status.hp ?? this.opponentStatus.hp
      this.opponentStatus.condition =
        status.condition ?? this.opponentStatus.condition
    }
  }
  ...
  setPlayerTexture(id?: string) {
    this.memberSprites.player.texture =
      id && (this.resources.getBack(id) as any)
  }
  ...
  particle(
    ...
    return {
      init: () => this.particles.push(new Type(this.particleStage, ...args)),
    }
  }
  ...
  sfx(
    ...
    return {
      init: state => {
        ...
        if (this.debug) {
          console.log(
            `View.sfx: sound started, name=${name} wait=${wait} panning=${panning}`
          )
        }
        ...
        sound.play({
          complete: () => {
            if (this.debug) {
              console.log(
                `View.sfx: sound completed, name=${name} wait=${wait} panning=${panning}`
              )
            }
            sound.filters = []
            state.waiting = false
    ...
    return Events.flatten([
      {
          ...
          this.fullStage.x = Math.floor(
            6 * Math.sin((2 * Math.PI * t * 1) / 30)
          )
          return t >= 30
        },
      },
