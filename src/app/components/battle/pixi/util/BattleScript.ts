...
function paralysisAnim(isPlayer: boolean): Script {
  const [x, y] = isPlayer ? [40, 76 + 20] : [128, 40 + 16]
  return [
    { do: 'WAIT', frames: 24 },
...
function confuseAnim(isPlayer: boolean): Script {
  const [x, y] = isPlayer ? [40, 42] : [128, 8]
  return [
    { do: 'WAIT', frames: 24 },
...
function freezeAnim(isPlayer: boolean): Script {
  const [x, y] = isPlayer ? [12, 104] : [160 - 32 - 24, 32 + 36]
  return [
    { do: 'WAIT', frames: 30 },
...
class BattleScript {
  ...
  constructor(
    ...
    this.introduction = (playerSwitch: Script) => (opponentSwitch: Script) => [
      ...
      [
        ...
        {
          do: 'TEXT',
          text: [this.battleInfo.info.opponent.name, 'wants to battle!'],
        },
        'CLEAR_TEXT',
    ...
    while (this.stream.length > 0) {
      ...
      switch (action[1]) {
        ...
        case '-prepare':
          ...
          break
        case '-activate':
          return this.handleActivate(action)
        case '-damage':
          return this.handleDamage(action)
        case '-start':
          return this.handleStart(action)
        case '-sidestart':
          return this.handleSidestart(action)
        case '-sideend':
          return this.handleSideend(action)
        case '-end':
          return this.handleEnd(action)
        case '-status':
          return this.handleStatus(action)
        case '-curestatus':
          return this.handleCureStatus(action)
        case '-heal':
          return this.handleHeal(action)
        case '-boost':
          return this.handleBoost(action, true)
        case '-unboost':
          return this.handleBoost(action, false)
        case '-setboost':
          return this.handleSetBoost(action)
        case '-singleturn':
          return this.handleSingleTurn(action)
        case '-singlemove':
          return this.handleSingleMove(action)
        case '-weather':
          return this.handleWeather(action)
        case '-mustrecharge':
          if (getIsPlayer(action[2]!)) {
            this.playerState.passing = true
          } else {
  ...
  private handleMove(action: string[]): Script {
    ...
    if (action[3] == null) {
      throw new Error(
        `BattleScript.handleMove: unexpected move, ${JSON.stringify(action)}`
      )
    }
    ...
    if (action[2] == null) {
      throw new Error(
        `BattleScript.handleMove: unexpected name, ${JSON.stringify(action)}`
      )
    }
  ...
  private performMove(move: string, isPlayer: boolean): Script {
    ...
    return [
      { do: 'LOAD_MOVE', move },
      isPlayer ? 'HIDE_PLAYER_STATS' : 'HIDE_OPPONENT_STATS',
      { do: 'WAIT', frames: 16 },
      ...
      isPlayer ? 'SHOW_PLAYER_STATS' : 'SHOW_OPPONENT_STATS',
      // post anim
      { do: 'EFFECT', name: move + '_POST', isPlayer },
      isPlayer && move === 'BATON PASS' ? 'FORCE_PLAYER_SWITCH' : null,
    ]
  }
  ...
  private effectiveness(): Script {
    ...
    return [
      ...
      this.moveResults.effectiveness && {
        do: 'TEXT',
        text: effectMessage[this.moveResults.effectiveness],
      },
    ]
  }
  ...
  private handleBoost(action: string[], direction: boolean): Script {
    if (action[2] == null || action[3] == null || action[4] == null) {
      throw new Error(
        `BattleScript.handleBoost: unexpected, ${JSON.stringify(action)}`
      )
    }
    ...
    if (Math.abs(amount) < 0.1) {
      const message = [
        `${target}'s`,
        `${statTxt} won't`,
        direction ? 'rise anymore!' : 'drop anymore!',
      ]
      return [
        { do: 'WAIT', frames: 60 },
    ...
    const message =
      amount > 1
        ? [
            `${target}'s`,
            `${statTxt}`,
            `${direction ? 'went way up' : 'sharply fell'}!`,
          ]
  ...
  private handleSetBoost(action: string[]): Script {
    if (action[2] == null || action[5] == null) {
      throw new Error(
        `BattleScript.handleSetBoost: unexpected, ${JSON.stringify(action)}`
      )
    }
    const name = getTextName(action[2])
    switch (getFrom(action[5])) {
      case 'move: Belly Drum':
        return {
          do: 'TEXT',
          text: [name, 'cut its HP and', 'maximized ATTACK!'],
        }
      default:
  ...
  private handleFaint(action: string[]): Script {
    if (action[2] == null) {
      throw new Error(
        `BattleScript.handleFaint: unexpected, ${JSON.stringify(action)}`
      )
    }
    ...
    if (isPlayer) {
      ...
      this.waitPlayerSwitchUntilOpponentFaints =
        isPlayer &&
        next != null &&
      ...
      if (this.playerState.index == null) {
        throw new Error(
          'BattleScript.handleFaint: playerState.index is undefined'
        )
      }
      return [
        ...
        this.waitPlayerSwitchUntilOpponentFaints || hasWon
          ? null
          : 'FORCE_PLAYER_SWITCH',
      ]
    }
    const playerSwitch = this.waitPlayerSwitchUntilOpponentFaints
      ? 'FORCE_PLAYER_SWITCH'
      : null
  ...
  private handleWin(action: string[]): Script {
    ...
    if (action[2] === this.playerState.trainerName) {
      return [
        ...
        {
          do: 'TEXT',
          text: [this.opponentState.trainerName!, 'was defeated!'],
        },
      ...
      return [
        ...
        {
          do: 'TEXT',
          text: [`${this.playerState.trainerName} whited`, 'out!'],
          auto: true,
        },
      ]
    }
  ...
  private findMember(team: MemberObject[], simulatorName: string): number {
    return team.findIndex(
      (member, index) => simulatorName === index.toString() + member.name
    )
  }
  ...
  sendOutPlayer(
    ...
    return [
      ...
      {
        do: 'TEXT',
        text: altText ? ['Go for it,', `${name}!`] : [`Go! ${name}!`],
        auto: true,
      },
      { do: 'WAIT', frames: 24 },
  ...
  sendOutOpponent(
    ...
    return [
      ...
      {
        do: 'TEXT',
        text: [`${this.battleInfo.info.opponent.name}`, 'sent out', `${name}!`],
        auto: true,
      },
      { do: 'WAIT', frames: 24 },
      { do: 'SFX', name: 'ballpoof' },
  ...
  private handleSwitch(action: string[], altText: boolean = false): Script {
    if (action[2] == null || action[3] == null || action[4] == null) {
      throw new Error(
        `BattleScript.handleSwitch: unexpected, ${JSON.stringify(action)}`
      )
    }
    ...
    if (action[2][1] === '1') {
      const index = this.findMember(
        this.battleInfo.info.player.team,
        simulatorName
      )
      return this.sendOutPlayer(
        name,
        index,
        status,
        this.playerState.fainted || altText
      )
    }
    const index = this.findMember(
      this.battleInfo.info.opponent.team,
      simulatorName
    )
  ...
  private handleCant(action: string[]): Script {
    if (action[2] == null) {
      throw new Error(
        `BattleScript.handleCant: unexpected, ${JSON.stringify(action)}`
      )
    }
    const name = getTextName(action[2])
    const isPlayer = getIsPlayer(action[2])
    switch (action[3]) {
      case 'recharge':
        return [{ do: 'TEXT', text: [`${name}`, 'must recharge!'] }]
      case 'par':
        return [{ do: 'TEXT', text: [`${name}'s`, 'fully paralyzed!'] }]
      case 'slp':
        return [
      ...
      case 'frz':
        return [{ do: 'TEXT', text: [name, 'is frozen solid!'] }]
      default:
        console.error('Unhandled:', action)
    }
    return []
  ...
  private handleActivate(action: string[]): Script {
    if (action[2] == null) {
      throw new Error(
        `BattleScript.handleActivate: unexpected, ${JSON.stringify(action)}`
      )
    }
    const name = getTextName(action[2])
    const isPlayer = getIsPlayer(action[2])
    const otherName = isPlayer
      ? 'Enemy ' + this.opponentState.name
    ...
    switch (action[3]) {
      ...
      case 'move: Fire Spin':
        return {
          do: 'TEXT',
          text: [name, 'was trapped!'],
        }
      case 'move: Destiny Bond':
        return {
          do: 'TEXT',
          text: [name, 'took down with it,', `${otherName}!`],
        }
      case 'Protect':
        return {
          do: 'TEXT',
          text: [`${name}'s`, 'PROTECTING itself!'],
        }
      case 'move: Beat Up':
        if (action[4] == null) {
          throw new Error(
            `BattleScript.handleActivate: Beat Up has no attacker, ${JSON.stringify(action)}`
          )
        }
      ...
      case 'move: Substitute':
        return {
          do: 'TEXT',
          text: ['The SUBSTITUTE', 'took damage for', `${name}!`],
        }
      case 'confusion':
        return [
  ...
  private handleDamage(action: string[]): Script {
    if (action[2] == null || action[3] == null) {
      throw new Error(
        `BattleScript.handleDamage: unexpected, ${JSON.stringify(action)}`
      )
    }
    const name = getTextName(action[2])
    const isPlayer = getIsPlayer(action[2])
    ...
    if (action[4] == null) {
      ...
      return [
        ...
        isPlayer
          ? 'SCREEN_SHAKE'
          : { do: 'EFFECT', name: 'FLICKER', isPlayer: false },
    ...
    switch (from) {
      case 'move: Whirlpool':
        ...
        break
      case 'move: Fire Spin':
        event = [{ do: 'TEXT', text: [`${name}'s`, 'hurt by', 'FIRE SPIN!'] }]
        break
      case 'confusion':
      ...
      case 'Curse':
        ...
        event = [{ do: 'TEXT', text: [`${name}'s`, 'hurt by the CURSE!'] }]
        break
      case 'Leech Seed':
        // TODO: leech seed effect
        event = [{ do: 'TEXT', text: ['LEECH SEED saps', `${name}!`] }]
        break
      case 'Spikes':
        event = [{ do: 'TEXT', text: [`${name}'s`, `hurt by SPIKES!`] }]
        break
      default:
        console.error('Unhandled:', action)
    }
    return [event, { do: 'HEALTH', isPlayer, hp: status.hp! }]
  }

  private handleStart(action: string[]): Script {
    if (action[2] == null) {
      throw new Error(
        `BattleScript.handleStart: unexpected, ${JSON.stringify(action)}`
      )
    }
    ...
    switch (condition) {
      ...
      case 'Curse': {
        if (action[4] == null) {
          throw new Error(
            `BattleScript.handleStart: curse is missing [of], ${JSON.stringify(action)}`
          )
        }
        return [
          {
  ...
  private handleSidestart(action: string[]): Script {
    if (action[2] == null) {
      throw new Error(
        `BattleScript.handleSidestart: unexpected, ${JSON.stringify(action)}`
      )
    }
    const isPlayer = getIsPlayer(action[2])
    ...
    switch (reason) {
      case 'Reflect':
        if (isPlayer) {
          return {
            do: 'TEXT',
            text: ['Your team is', 'stronger against', 'physical moves!'],
          }
        }
        return {
          do: 'TEXT',
          text: [
            `${this.opponentState.trainerName}'s`,
            'team is stronger',
            'against physical',
            'moves!',
          ],
      ...
      case 'move: Light Screen':
        if (isPlayer) {
          return {
            do: 'TEXT',
            text: ['Your team is', 'stronger against', 'special moves!'],
          }
        }
        return {
          do: 'TEXT',
  ...
  private handleSideend(action: string[]): Script {
    if (action[2] == null) {
      throw new Error(
        `BattleScript.handleSideend: unexpected, ${JSON.stringify(action)}`
      )
    }
    const isPlayer = getIsPlayer(action[2])
    const reason = action[3]
    switch (reason) {
      case 'Reflect':
        if (isPlayer) {
          return {
            do: 'TEXT',
            text: [`${this.playerState.name}'s`, 'REFLECT faded!'],
          }
        }
        return {
          do: 'TEXT',
          text: [`Enemy ${this.opponentState.name}'s`, 'REFLECT faded!'],
        }
      case 'move: Light Screen':
        if (isPlayer) {
          return {
            do: 'TEXT',
            text: [`${this.playerState.name}'s`, 'LIGHT SCREEN faded!'],
          }
        }
        return {
          do: 'TEXT',
          text: [`Enemy ${this.opponentState.name}'s`, 'LIGHT SCREEN faded!'],
        }
      case 'Spikes':
        if (isPlayer) {
          ...
          return {
            do: 'TEXT',
            text: [`${this.playerState.name}`, 'blew away SPIKES!'],
          }
        }
        return {
          do: 'TEXT',
          text: [`Enemy ${this.opponentState.name}`, 'blew away SPIKES!'],
        }
      default:
        console.error('Unhandled:', action)
    }
  ...
  private handleEnd(action: string[]): Script {
    if (action[2] == null) {
      throw new Error(
        `BattleScript.handleEnd: unexpected, ${JSON.stringify(action)}`
      )
    }
    const name = getTextName(action[2])
    const isPlayer = getIsPlayer(action[2])
  ...
  private handleSingleTurn(action: string[]): Script {
    if (action[2] == null) {
      throw new Error(
        `BattleScript.handleSingleTurn: unexpected, ${JSON.stringify(action)}`
      )
    }
    const name = getTextName(action[2])
    switch (action[3]) {
  ...
  private handleSingleMove(action: string[]): Script {
    if (action[2] == null) {
      throw new Error(
        `BattleScript.handleSingleMove: unexpected, ${JSON.stringify(action)}`
      )
    }
    const name = getTextName(action[2])
    switch (action[3]) {
      case 'Destiny Bond':
        return {
          do: 'TEXT',
          text: [`${name}'s`, 'trying to take its', 'opponent with it!'],
        }
      default:
        console.error('Unhandled:', action)
    }
  ...
  private handleWeather(action: string[]): Script {
    if (action[2] == null) {
      throw new Error(
        `BattleScript.handleWeather: unexpected, ${JSON.stringify(action)}`
      )
    }
    if (action[3] === '[upkeep]') {
      switch (action[2]) {
  ...
  private handleStatus(action: string[]): Script {
    if (action[2] == null) {
      throw new Error(
        `BattleScript.handleStatus: unexpected, ${JSON.stringify(action)}`
      )
    }
    const condition = action[3]
    const isPlayer = getIsPlayer(action[2])
    ...
    switch (condition) {
      ...
      case 'par':
        ...
        script.push({
          do: 'TEXT',
          text: [`${name}'s`, 'paralyzed! Maybe', "it can't attack!"],
        })
        break
      case 'brn':
  ...
  private handleCureStatus(action: string[]): Script {
    if (action[2] == null) {
      throw new Error(
        `BattleScript.handleCureStatus: unexpected, ${JSON.stringify(action)}`
      )
    }
    const name = getTextName(action[2])
    const isPlayer = getIsPlayer(action[2])
  ...
  private handleHeal(action: string[]): Script {
    if (action[2] == null || action[3] == null) {
      throw new Error(
        `BattleScript.handleHeal: unexpected, ${JSON.stringify(action)}`
      )
    }
    const name = getTextName(action[2])
    const isPlayer = getIsPlayer(action[2])
    ...
    if (action[4] === '[silent]') {
      return [{ do: 'HEALTH', hp: status.hp, isPlayer }]
    }
    const from = getFrom(action[4])
    switch (from) {
      case 'drain':
        if (action[5] == null) {
          throw new Error(
            `BattleScript.handleHeal: drain is missing [of], ${JSON.stringify(action)}`
          )
        }
        return [
